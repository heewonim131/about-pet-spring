...
function moisPostList(targetId, callBack){
	...
	var options = {
		...
		, dataType : "jsonp"
		, data : {
			confmKey : viewJsonData.fo_mois_post_confmKey,
			...
			, done : function(data){
				...
				if(data.rtnCode == 'S'){
					if(nowBuyYn == "Y"){
						...
						ui.toast('<div class="link"><p class="tt">장바구니에 추가되었어요</p><a href="/order/indexCartList" class="lk" data-content="" data-url="/order/indexCartList">바로가기</a></div>');
	...
	$(document).ready(function(e) {
		$(document)
			...
			.on("click", ".btn-with-prd, .log_connectTingWrap .btn_connectTing", function(e) {
				...
				if($this.hasClass("btn-with-prd"))  getRelatedGoodsWithTv($this, $this.data("content"));
				else                                getRelatedGoodsWithPetLog($this, $this.data("content"));
			})
			...
			.on("click", '[data-target="goods"][data-action="interest"], #goodsSummary .btZZim, .cartNavs .btZZim ', function(e) {
				...
				let deviceGb = $btn.data("device-gb");
				console.log('[data-target="goods"][data-action="interest"] click '+"goodsId: "+goodsId);
				if(goodsId) {
							...
							if(data.actGubun =='login'){
								...
								ui.confirm('로그인 후 서비스를 이용할 수 있어요.<br>로그인 할까요?', { // 컨펌 창 옵션들
									ycb: function () {
										...
										if(deviceGb == "APP" && document.location.href.indexOf("/tv/series/indexTvDetail") > -1){
								...
								let msg = "";
								if(deviceGb == "APP" && document.location.href.indexOf("/tv/series/indexTvDetail") > -1){
									msg = '<div class="link"><p class="tt">찜리스트에 추가되었어요</p><a href="#" onclick="fncMoveZzim(); return false;" class="lk" data-content="" data-url="/mypage/shop/myWishList">바로가기</a></div>';
								}else{
									msg = '<div class="link"><p class="tt">찜리스트에 추가되었어요</p><a href="/mypage/shop/myWishList" onclick="fncMoveZzim(); return false;" class="lk" data-content="" data-url="/mypage/shop/myWishList">바로가기</a></div>';
								}
								ui.toast(msg,{
									// sec:1500000,
									bot:77
				...
				e.preventDefault();
				return false;
			}).off("click", "#goodsRelatedBottomListNCart .cart")
			.on("click", "#goodsRelatedBottomListNCart .cart", function(e) {
				//if( $(".popToast").is(":visible") ) return;

				let $this = $(this);
				let goodsId = $this.data("content");
				let itemNo = $this.data("item-no");
				let dlgtSubGoodsId = $this.data("dlgt-sub-goods-id");
				let cstrtTp = $this.data("cstrt-tp");
				let goodsIdStr = "::";
				let ordMkiYn = $this.data("ord-mki-yn");

				console.log("goodsId: "+goodsId);
				console.log("itemNo: "+itemNo);
				console.log("dlgtSubGoodsId: "+dlgtSubGoodsId);
				console.log("cstrtTp: "+cstrtTp);
				...
				if(ordMkiYn == "Y") {
					...
					if(cstrtTp == "ITEM" || cstrtTp == "SET") {
						goodsIdStr = goodsId+":"+itemNo+":";
						console.log("goodsIdStr["+cstrtTp+"]: "+goodsIdStr);
						commonFunc.insertCart(goodsIdStr, 1, 'N');
					}else if(cstrtTp == "PAK") {
						goodsIdStr = dlgtSubGoodsId+":"+itemNo+":"+goodsId;
						console.log("goodsIdStr["+cstrtTp+"]: "+goodsIdStr);
						commonFunc.insertCart([goodsIdStr], [1], 'N');
					}else if(cstrtTp == "ATTR") {
						goodsIdStr = dlgtSubGoodsId+":"+itemNo+":"+goodsId;
						console.log("goodsIdStr["+cstrtTp+"]: "+goodsIdStr);
						commonFunc.insertCart(goodsIdStr, 1, 'N');
					}else{
...
function getRelatedGoodsWithTv($btn, vdId) {
	$btn = $($btn);
	$("body").addClass("isCartNavs");
	callRelatedGoodAjax($btn, "/goods/popupGoodsRelated/tv/"+vdId);
	//ui.dim.set();
}
function getRelatedGoodsWithPetLog($btn, petLogNo) {
	$btn = $($btn);
	$("body").addClass("isCartNavs");
...
function callRelatedGoodAjax($btn, url) {
	if($("#goodsRelated").length > 0 && relatedGoodsCallback != undefined && typeof(relatedGoodsCallback) == 'function') {
		relatedGoodsCallback($btn);
	}else{
		waiting.start();
