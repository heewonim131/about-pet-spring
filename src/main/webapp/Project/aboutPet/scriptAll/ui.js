...
//		this.input_viewPort.init();
	...
	ly:{
		...
		contHeight:function(){
			...
			// mobile tab list 최소높이값
			var uiTH = $(".uiTab:visible").outerHeight() || 0;
			var fakePopTH = $(".fake-pop.top:visible").outerHeight() || 0;
			var tabListH = winH - headCuH - footH - uiTH - fakePopTH - 15;
			$('.isMo .uiTab_content > ul > li').css({"min-height":tabListH});
			$('.isMo .commentBoxAp .uiTab_content > ul > li').removeAttr("style"); /* APETQA-6101 210701 lju02 */
		...
		bnr:{
			...
			using: function() {
				...
				$(this.els).each(function(i){
					...
					if( num >= 2) {
						_this.slide[i] = new Swiper($(this), _this.opt);
					}else{
						$(this).find(".sld-nav").hide();
					}
				});
			}
		}
	...
	disp:{ // 전시 
		...
		sld:{ // 샵 메인 슬라이드UI 모음
			init:function(){
				...
				//$(this.gogos.els).length && this.gogos.using(); //21.06.09 APET-1101 lcm01 임시로 주석처리 중 
				//$(this.gogos2.els).length && this.gogos2.using(); //21.06.09 APET-1101 lcm01 추가  임시로 주석처리 중
				...
				$(this.bests.els).length && this.bests.using();
			...
			/* 21.06.09 APET-1101 lcm01 : 임시로 주석처리 중
			gogos:{ // 바로가기
				els:".mn_gogos_sld .swiper-container",
				opt: {
					observer: true,
					observeParents: true,
					watchOverflow:true,
			...
			/*gogos2:{ // 21.06.09 APET-1101 lcm01 : 바로가기 아이콘 9개 이상
				els:".shortcuts9 .swiper-container",
				opt: {
					//observer: true,
					//observeParents: true,
					//watchOverflow:true,
			...
			often:{ // 자주구매한상품 
				...
				opt: {
					...
					spaceBetween: 30,
					navigation: {
						nextEl: '.mn_often_sld .sld-nav .bt.next',
						prevEl: '.mn_often_sld .sld-nav .bt.prev',
					},
					breakpoints: {
						1024: {
							spaceBetween:12,
			...
			recom:{ // 맞춤추천 
				...
				opt: {
					...
					spaceBetween: 30,
					navigation: {
						nextEl: '.mn_recom_sld .sld-nav .bt.next',
						prevEl: '.mn_recom_sld .sld-nav .bt.prev',
					},
					breakpoints: {
						1024: {
							spaceBetween: 10,
				...
				using: function() {		// APET-1079 중복 슬라이드 210615 lju02
					$(".mn_recom_sld  .swiper-container").each(function(i,n){
						ui.disp.sld.discn.slide.push(new Swiper($(n), {
							observer: true,
							observeParents: true,
							watchOverflow:true,
							simulateTouch:false,
			...
			discn:{ // 할인상품 
				...
				opt: {
					...
					spaceBetween: 30,
					navigation: {
						nextEl: '.mn_discn_sld .sld-nav .bt.next',
						prevEl: '.mn_discn_sld .sld-nav .bt.prev',
					},
					breakpoints: {
						1024: {
							spaceBetween: 10,
			...
			bests:{ // 베스트
				els:".mn_bests_sld .swiper-container",
				opt:{
					observer: true,
					observeParents: true,
					watchOverflow:true,
	...
	popSelect:{ // 셀렉트 메뉴 팝업
		...
		open:function(name,tit,list,sel){
			...
			$(".pop-select").fadeIn(100,function(){	
				...
				ui.lock.using(true); //2021.06.28 추가함
			});
			// ui.lock.using(true);
		...
		createAddSource : function(el){
			...
			function auto_h(e){			// textarea 제어
				...
				setTimeout(function(){
					...
					var fixedBox_bottom = (4 + $target.closest(".commentBoxAp").find(".con > .input").innerHeight());
					...
					if((check_mo === false) && check_type1){
						...
						$target.closest(".commentBoxAp").find(".con > .box").css("padding-top",inputH);
					}
				...
				if(check) {
					...
					$(window).scroll(function(e){
						...
						if(logCheck){
							...
							if(scroll > point_min && $(".sticky_filter_top").length == 0){
								header.css({
